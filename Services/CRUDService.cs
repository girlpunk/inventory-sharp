...
public abstract class CRUDService<T>(IServiceProvider services) : DbServiceBase<AppDbContext>(services), ICRUDService<T> where T : BaseModel
{
    /// <inheritdoc />
    ...
    /// </summary>
    public abstract Func<AppDbContext, DbSet<T>> DbSet { get; }
    ...
    public virtual async Task<T> Create(CreateCommand<T> command, CancellationToken cancellationToken = default)
    {
        ...
        DbOperationScope<AppDbContext>.GetOrCreate(context).Require();
